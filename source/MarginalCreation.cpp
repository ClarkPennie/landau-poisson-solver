...
double f_marg_Homo_L(double *U, int j1, int j2, double v1, double v2)
{
	...
	v1_dif = v1 - Gridv_L((double)j1);															// set v1_dif to v1 - v_j1
	v2_dif = v2 - Gridv_L((double)j2);															// set v2_dif to v2 - v_j2
	v_squares = v1_dif*v1_dif + v2_dif*v2_dif;													// set v_squares to (v1 - v_j1)^2 + (v2 - v_j2)^2
	k0 = j1*Nv*Nv + j2*Nv;																		// set k0 to j1*Nv^2 +j2*Nv
	retn = 0;																					// initialise fM_val at 0, since the value is calculated through a sum
	...
	{
		k = k0 + j3;																			// set k to i*Nv^3 + j1*Nv^2 + j2*Nv + j3
		retn += dv_L*U[6*k+0] + U[6*k+2]*v1_dif + U[6*k+3]*v2_dif
						+ U[6*k+5]*(v_squares/dv_L +dv_L/12);										// add dv*U[6*k+0] + U[6*k+2]*v1_dif + U[6*k+3]*v2_dif + U[6*k+5]*((v1_dif*v1_dif + v2_diff*v2_diff)/dv +dv/12) for the given j3 in the sum for retn
	}
	return retn;																				// return the value of the marginal evaluated at x & v1
}
...
double f_marg_Homo_H(double *U, int j1, int j2, double v1, double v2)
{
	...
	v1_dif = v1 - Gridv_H((double)j1);															// set v1_dif to v1 - v_j1
	v2_dif = v2 - Gridv_H((double)j2);															// set v2_dif to v2 - v_j2
	v_squares = v1_dif*v1_dif + v2_dif*v2_dif;													// set v_squares to (v1 - v_j1)^2 + (v2 - v_j2)^2
	k0 = j1*Nv*Nv + j2*Nv;																		// set k0 to j1*Nv^2 +j2*Nv
	retn = 0;																					// initialise fM_val at 0, since the value is calculated through a sum
	...
	{
		k = k0 + j3;																			// set k to i*Nv^3 + j1*Nv^2 + j2*Nv + j3
		retn += dv_H*U[6*k+0] + U[6*k+2]*v1_dif + U[6*k+3]*v2_dif
						+ U[6*k+5]*(v_squares/dv_H +dv_H/12);										// add dv*U[6*k+0] + U[6*k+2]*v1_dif + U[6*k+3]*v2_dif + U[6*k+5]*((v1_dif*v1_dif + v2_diff*v2_diff)/dv +dv/12) for the given j3 in the sum for retn
	}
	return retn;																				// return the value of the marginal evaluated at x & v1
}
...
void PrintMarginalLoc_Homo_Multispecies(FILE *margfile_L, FILE *margfile_H)															// function to print the values of x & v1 which the marginal will be evaluated at in the first two rows of the file with tag margfile (subsequent rows will be the values of the marginal at given timesteps)
{
	...
	double v1_0_H, v1_val_H, v2_0_H, v2_val_H, ddv_H;											// declare v1_0 (the v1 value at the left edge of a given cell), v1_val (the v1 value to be evaluated at), v2_0 (the v2 value at the left edge of a given cell), v2_val (the v2 value to be evaluated at) & ddv (the space between v values)

	np = 4;																						// set np to 4
	ddv_L = dv_L/np;																			// set ddv to the velocity cell width divided by np
	ddv_H = dv_H/np;																			// set ddv to the velocity cell width divided by np
	for(j1=0; j1<Nv; j1++)
	{
		v1_0_L = Gridv_L((double)j1 - 0.5);														// set v1_0 to the value of v1 at the left edge of the j1-th space cell
		v1_0_H = Gridv_H((double)j1 - 0.5);														// set v1_0 to the value of v1 at the left edge of the j1-th space cell
		for (nv1=0; nv1<np; nv1++)
		{
			v1_val_L = v1_0_L + nv1*ddv_L;														// set v1_val to v1_0 plus nv1 increments of width ddv
			v1_val_H = v1_0_H + nv1*ddv_H;														// set v1_val to v1_0 plus nv1 increments of width ddv
			for(j2=0; j2<Nv; j2++)
			{
				for (nv2=0; nv2<np; nv2++)
				{
					fprintf(margfile_L, "%11.8g  ", v1_val_L);									// in the file tagged as fmarg, print the v1 coordinate
					fprintf(margfile_H, "%11.8g  ", v1_val_H);									// in the file tagged as fmarg, print the v1 coordinate
				}
			}
		}
	}
	...
	for(j1=0; j1<Nv; j1++)
	{
		for (nv1=0; nv1<np; nv1++)
		{
			for(j2=0; j2<Nv; j2++)
			{
				v2_0_L = Gridv_L((double)j2 - 0.5);												// set v2_0 to the value of v2 at the left edge of the j2-th velocity cell in the v2 direction
				v2_0_H = Gridv_H((double)j2 - 0.5);												// set v2_0 to the value of v2 at the left edge of the j2-th velocity cell in the v2 direction
				for (nv2=0; nv2<np; nv2++)
				{
					v2_val_L = v2_0_L + nv2*ddv_L;												// set v2_val to v2_0 plus nv2 increments of width ddv
					v2_val_H = v2_0_H + nv2*ddv_H;												// set v2_val to v2_0 plus nv2 increments of width ddv
					fprintf(margfile_L, "%11.8g  ", v2_val_L);									// in the file tagged as fmarg, print the v2 coordinate
					fprintf(margfile_H, "%11.8g  ", v2_val_H);									// in the file tagged as fmarg, print the v2 coordinate
				}
			}
		}
	}
...
void PrintMarginal_Homo_Multispecies(double *U_L, double *U_H, FILE *margfile_L, FILE *margfile_H)													// function to print the values of the marginal in the file tagged as margfile at the given timestep
{
	...
	double v1_0_H, v1_val_H, v2_0_H, v2_val_H, fM_val_H, ddx_H, ddv_H;							// declare v1_0 (the v1 value at the left edge of a given cell), v1_val (the v1 value to be evaluated at), v2_0 (the v2 value at the left edge of a given cell), v2_val (the v2 value to be evaluated at), fM_val (the value of the marginal evaluated at (x_val, v1_val), ddx (the space between x values) & ddv (the space between v1 values)
	...
	for(j1=0; j1<Nv; j1++)
	{
		v1_0_L = Gridv_L((double)j1 - 0.5);														// set v1_0 to the value of v1 at the left edge of the j1-th velocity cell in the v1 direction
		v1_0_H = Gridv_H((double)j1 - 0.5);														// set v1_0 to the value of v1 at the left edge of the j1-th velocity cell in the v1 direction
		for (nv1=0; nv1<np; nv1++)
		{
			v1_val_L = v1_0_L + nv1*ddv_L;														// set v1_val to v1_0 plus nv increments of width ddv
			v1_val_H = v1_0_H + nv1*ddv_H;														// set v1_val to v1_0 plus nv increments of width ddv
			for(j2=0; j2<Nv; j2++)
			{
				v2_0_L = Gridv_L((double)j2 - 0.5);												// set v2_0 to the value of v2 at the left edge of the j2-th velocity cell in the v2 direction
				v2_0_H = Gridv_H((double)j2 - 0.5);												// set v2_0 to the value of v2 at the left edge of the j2-th velocity cell in the v2 direction
				for (nv2=0; nv2<np; nv2++)
				{
					...
					fM_val_L = f_marg_Homo_L(U_L, j1, j2, v1_val_L, v2_val_L);					// calculate the value of the marginal, evaluated at x_val & v1_val by using the function in the space cell i and velocity cell j1 in the v1 direction
					fM_val_H = f_marg_Homo_H(U_H, j1, j2, v1_val_H, v2_val_H);					// calculate the value of the marginal, evaluated at x_val & v1_val by using the function in the space cell i and velocity cell j1 in the v1 direction
					fprintf(margfile_L, "%11.8g  ", fM_val_L);									// in the file tagged as fmarg, print the value of the marginal f_M(t, x, v1)
					fprintf(margfile_H, "%11.8g  ", fM_val_H);									// in the file tagged as fmarg, print the value of the marginal f_M(t, x, v1)
				}
			}
		}
	}
